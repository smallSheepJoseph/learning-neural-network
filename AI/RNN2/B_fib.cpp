...
struct layer_RNN_record{
    ...
    static constexpr int TimeBUF = Time; // for absolutely nothing
...
struct layer_RNN{
    ...
    void run(Record& target_rd,dl (*input)[InSize],dl (*output)[OutSize],int len){
        ...
        for(int t = 0;t<len;t++){
            memcpy(target_rd.Vinput[t]+MemorySize,input[t],InSize*sizeof(dl));
...
struct model{
    using model_type = layer_RNN<1,1,25,RNN_PRED_LEN>;
