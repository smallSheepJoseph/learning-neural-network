...
struct memory_allocer{
    dl *w_block, *g_block;
    int begin,end;
    int ptr;
    ParamBlock *PB_arr;
    int PB_top = 0,PB_size;
    memory_allocer(int size,int _PB_size){
...
struct GradientFixer{
    ...
    void fix(){
        ...
        dl g2 = 0;
        #pragma omp parallel for simd reduction(+:g2) if(Size > 10000)
        for(int i = 0;i<Size;i++){
            g2 += _grad[i]*_grad[i];
        }
        dl g = sqrt(g2);
...
struct NN{
    static constexpr int input_len = InSize, hidden_len = HiddenSize, output_len = OutSize, _depth = depth;
...
struct NN_data{
    static constexpr int input_len = InSize, output_len = OutSize;
    dl x[InSize];
    dl y[OutSize];
};
...
struct model{
    using model_type = NN<2,15,1,5>;
    static constexpr int memsize = model_type::memory_required;
    static constexpr int total_size = memsize + 10;
    using Data = NN_data<model::model_type::input_len,model::model_type::output_len>;
    TrainConfig& config;
    memory_allocer mal;
    model_type nn;
    ...
    dl train(Data& d){
        return nn.train(d.x,d.y);
    }

    void run(dl* in,dl* out){
        return nn.run(in,out);
    }

    void run(Data& d,dl* out){
        return nn.run(d.x,out);
    }
...
// RNN not done yet
// template<int InSize,int OutSize,int MemorySize,int DataLen,int depth>
// struct layer_RNN{
//     // using Self = RNN<InSize, HiddenSize, OutSize, MemorySize, DataLen, depth>;
//     Layer<InSize+MemorySize,OutSize+MemorySize> ly;
...
// Data<RNN_PRED_LEN> gen_data(dl a,dl b,dl rd=0.0){
//     Data<RNN_PRED_LEN> d;
...
void gen_data(Data& d){
    d.x[0] = random_z()*10-5;
    d.x[1] = random_z()*10-5;
    d.y[0] = fixed_sin((d.x[0]+d.x[1])*0.5);
}
...
int main(){
    ...
    for(int i = 0;i<n;i++){
        gen_data(train_data[i]);
    }
    tcf.enable_fix_gradient = true;
    tcf.lr = 0.002;
    // tcf.clip_low = 1.0;
    ll train_counter = 0;
    //notice that this part is reuse-able for NN
    for(int t = 0;t<200;t++){
        if(t<20){
            tcf.lr = 0.00001*(1 + (dl)t);
        }else{
            tcf.lr = 0.0005 / (1 + (dl)t);
        }
        for(int i = 0;i<n;i++){
            auto [return_id,batch_id] = hsm.get_index(2);
            dl hardness = m.train(train_data[return_id]);
        ...
        for(int i = 0;i<min(100,n);i++){
            dl tmp[model::model_type::output_len]; //model::model_type::output_len is a constexpr
            m.run(train_data[i],tmp);
            for(int j = 0;j<model::model_type::output_len;j++){
                loss += 2.0*(tmp[j]-train_data[i].y[j])*(tmp[j]-train_data[i].y[j]);
            }
        }
        loss /= 100.0*model::model_type::output_len;
        printf("st: %d, loss = %lf\n",t,loss);
    }


    //this part can only use on this model
    while(1){
        dl input[2];
        dl pred[1];
        printf("input a,b:\n");
        ...
        m.run(input,pred);
        printf("\n pred: %lf, real: %lf\n",pred[0],fixed_sin((input[0]+input[1])*0.5));
    }
